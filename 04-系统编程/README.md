...
HTTP协议全称超文本传输协议(HyperText Transfer Protocol)，是一种用于分布式、协作式和超媒体信息系统的**应用层协议**。HTTP是万维网的数据通信的基础。

#### 协议特点

HTTP协议是基于传输层TCP协议之上的应用层协议，但是它和TCP这种长连接协议有所不同，它的特点有如下:

- **请求/响应**模型。

  即每建立一次HTTP连接，只能发送一次数据，并且获得一次响应。并且请求必定是由客户端发出，而服务器负责应答。

- **无连接**

  这里的无连接只是一种比喻，其含义就是每次连接只处理一个请求，处理完应答后就关闭连接，所以每个连接时长都特别短，称之为**短连接**。

  采用这种方式可以**节省传输时间**，并且提高并发性能，不需要为每个客户端建立长久的连接。

- **无状态**

  因为是采用短连接(**请求/响应**)模型，所以每次会话结束后将不会保留任何状态，无法得知在此之前的所有会话信息。
